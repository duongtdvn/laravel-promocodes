...
class Promocodes
{
    ...
    {
        $this->codes = Promocode::query()->pluck('code')->toArray();
    ...
    public function create($amount = 1, $reward = null, array $data = [], $expires_in = null, $is_disposable = false)
    {
        ...
        foreach ($this->output($amount) as $code) {
            $records[] = [
                ...
                'expired_at' => $expires_in ? Carbon::now()->addDays($expires_in) : null,
                'is_disposable' => $is_disposable,
            ];
        }
        if (Promocode::query()->insert($records)) {
    ...
    {
        // TODO: consider to remove used code or not?
